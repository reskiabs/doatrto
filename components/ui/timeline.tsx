...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div className="w-full bg-white font-sans md:px-10" ref={containerRef}>
      ...
      <div ref={ref} className="relative max-w-[1140px] mx-auto pb-20">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex justify-between pt-10 md:pt-40 md:gap-10"
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-[555px] md:w-full">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-black flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-neutral-200 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 p-2" />
              </div>
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-bold bg-gradient-to-r from-seondary  via-tertiary to-primary bg-clip-text text-transparent dark:text-neutral-500">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 max-w-[555px] pr-4 md:pl-4 w-full">
